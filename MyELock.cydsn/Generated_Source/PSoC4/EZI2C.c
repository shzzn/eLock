...
static void EZI2C_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void EZI2C_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            EZI2C_RX_SDA_MOSI_HSIOM_SEL_GPIO,
            EZI2C_TX_SCL_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (EZI2C_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[EZI2C_RX_SDA_MOSI_PIN_INDEX] = EZI2C_RX_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [EZI2C_RX_SDA_MOSI_PIN_INDEX] = EZI2C_PIN_DM_OD_LO;
        #elif (EZI2C_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[EZI2C_RX_WAKE_SDA_MOSI_PIN_INDEX] = EZI2C_RX_WAKE_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [EZI2C_RX_WAKE_SDA_MOSI_PIN_INDEX] = EZI2C_PIN_DM_OD_LO;
        #else
        #endif /* (EZI2C_RX_SDA_MOSI_PIN) */
        
        #if (EZI2C_TX_SCL_MISO_PIN)
            hsiomSel[EZI2C_TX_SCL_MISO_PIN_INDEX] = EZI2C_TX_SCL_MISO_HSIOM_SEL_I2C;
            pinsDm  [EZI2C_TX_SCL_MISO_PIN_INDEX] = EZI2C_PIN_DM_OD_LO;
        ...
        {
        #if (EZI2C_RX_SDA_MOSI_PIN)
            hsiomSel[EZI2C_RX_SDA_MOSI_PIN_INDEX] = EZI2C_RX_SDA_MOSI_HSIOM_SEL_SPI;
        #elif (EZI2C_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[EZI2C_RX_WAKE_SDA_MOSI_PIN_INDEX] = EZI2C_RX_WAKE_SDA_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (EZI2C_RX_SDA_MOSI_PIN) */
        
        #if (EZI2C_TX_SCL_MISO_PIN)
            hsiomSel[EZI2C_TX_SCL_MISO_PIN_INDEX] = EZI2C_TX_SCL_MISO_HSIOM_SEL_SPI;
        #endif /* (EZI2C_TX_SCL_MISO_PIN) */
            ...
            {
                /* Slave */
                pinsDm[EZI2C_RX_SDA_MOSI_PIN_INDEX] = EZI2C_PIN_DM_DIG_HIZ;
                pinsDm[EZI2C_TX_SCL_MISO_PIN_INDEX] = EZI2C_PIN_DM_STRONG;
                ...
                /* Disable input buffer */
                 pinsInBuf |= EZI2C_TX_SCL_MISO_PIN_MASK;
            #endif /* (EZI2C_TX_SCL_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[EZI2C_RX_SDA_MOSI_PIN_INDEX] = EZI2C_PIN_DM_STRONG;
                pinsDm[EZI2C_TX_SCL_MISO_PIN_INDEX] = EZI2C_PIN_DM_DIG_HIZ;
            ...
            #if (EZI2C_RX_SDA_MOSI_PIN)
                pinsInBuf |= EZI2C_RX_SDA_MOSI_PIN_MASK;
            #elif (EZI2C_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= EZI2C_RX_WAKE_SDA_MOSI_PIN_MASK;
            #else
            #endif /* (EZI2C_RX_SDA_MOSI_PIN) */
        ...
        else /* UART */
        {
            if (EZI2C_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (EZI2C_TX_SCL_MISO_PIN)
                hsiomSel[EZI2C_TX_SCL_MISO_PIN_INDEX] = EZI2C_TX_SCL_MISO_HSIOM_SEL_UART;
                pinsDm  [EZI2C_TX_SCL_MISO_PIN_INDEX] = EZI2C_PIN_DM_OD_LO;
            #endif /* (EZI2C_TX_SCL_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (EZI2C_RX_SDA_MOSI_PIN)
                    hsiomSel[EZI2C_RX_SDA_MOSI_PIN_INDEX] = EZI2C_RX_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [EZI2C_RX_SDA_MOSI_PIN_INDEX] = EZI2C_PIN_DM_DIG_HIZ;
                #elif (EZI2C_RX_WAKE_SDA_MOSI_PIN)
                    hsiomSel[EZI2C_RX_WAKE_SDA_MOSI_PIN_INDEX] = EZI2C_RX_WAKE_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [EZI2C_RX_WAKE_SDA_MOSI_PIN_INDEX] = EZI2C_PIN_DM_DIG_HIZ;
                #else
                #endif /* (EZI2C_RX_SDA_MOSI_PIN) */
                }
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        EZI2C_SET_HSIOM_SEL(EZI2C_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       EZI2C_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[EZI2C_RX_SDA_MOSI_PIN_INDEX]);

        EZI2C_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[EZI2C_RX_SDA_MOSI_PIN_INDEX]);

        #if (!EZI2C_CY_SCBIP_V1)
            EZI2C_SET_INP_DIS(EZI2C_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                         EZI2C_uart_rx_i2c_sda_spi_mosi_MASK,
                                         (0u != (pinsInBuf & EZI2C_RX_SDA_MOSI_PIN_MASK)));
        #endif /* (!EZI2C_CY_SCBIP_V1) */
        ...
        EZI2C_SET_HSIOM_SEL(EZI2C_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[EZI2C_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        EZI2C_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[EZI2C_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        EZI2C_SET_INP_DIS(EZI2C_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     EZI2C_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & EZI2C_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        EZI2C_SET_INCFG_TYPE(EZI2C_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        EZI2C_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (EZI2C_TX_SCL_MISO_PIN)
        EZI2C_SET_HSIOM_SEL(EZI2C_TX_SCL_MISO_HSIOM_REG,
                                       EZI2C_TX_SCL_MISO_HSIOM_MASK,
                                       EZI2C_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[EZI2C_TX_SCL_MISO_PIN_INDEX]);

        EZI2C_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[EZI2C_TX_SCL_MISO_PIN_INDEX]);

    #if (!EZI2C_CY_SCBIP_V1)
        EZI2C_SET_INP_DIS(EZI2C_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     EZI2C_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & EZI2C_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!EZI2C_CY_SCBIP_V1) */
    #endif /* (EZI2C_RX_SDA_MOSI_PIN) */
